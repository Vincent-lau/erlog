...
work(State =
  ...
  try erpc:call(?coor_node, coordinator, assign_task, [node()], 5000) of
    T = #task{type = evaluate,
      ...
      frag:hash_frag(
        dbs:subtract(NewFullDB, FullDB), Program, ProgNum, 1, NumTasks, TmpPath ++ "fulldb"),
      % call finish task on coordinator
      finish_task(T),
      ?LOG_INFO("~p rpc results for finish at stage ~p task ~p ~n",
    ...
    exit:{exception, Reason} -> % exception from the gen_server side
      ?LOG_INFO("~p gen_server call timed out with reason ~p, trying again~n",
                [node(), Reason]),
      work(State)
  end.
